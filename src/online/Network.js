...
//
// The first page of results is cached in sessionStorage for a few minutes, so
// switching away from the Online tab and back (or reopening it later in the
// same session) doesn't spend another call against GitHub's 60/hour limit —
// hitting Refresh always bypasses the cache and fetches fresh.
...
var NETWORK_CACHE_KEY = 'vstuffNetworkCache';
var NETWORK_CACHE_TTL_MS = 3 * 60 * 1000;

var networkAllProjects = [];
var networkNextPageUrl = null;

function networkReadCache() {
  try {
    var raw = sessionStorage.getItem(NETWORK_CACHE_KEY);
    if (!raw) return null;
    var cached = JSON.parse(raw);
    if (!cached || typeof cached.timestamp !== 'number') return null;
    if (Date.now() - cached.timestamp > NETWORK_CACHE_TTL_MS) return null;
    return cached;
  } catch (err) {
    return null;
  }
...
function networkWriteCache(projects, nextPageUrl) {
  try {
    sessionStorage.setItem(NETWORK_CACHE_KEY, JSON.stringify({
      timestamp: Date.now(),
      projects: projects,
...
function networkFormatRelativeDate(iso) {
  ...
  return '' +
    '<a class="online-card" href="' + networkEscapeHtml(issue.html_url) + '" target="_blank" rel="noopener noreferrer">' +
      ...
      '<div class="online-card-footer">' +
        '<div class="online-card-date" title="' + networkEscapeHtml(networkFormatDate(issue.created_at)) + '">' + networkEscapeHtml(networkFormatRelativeDate(issue.created_at)) + '</div>' +
...
function loadCommunityProjects(force) {
  var grid = document.getElementById('onlineGrid');
  if (!grid) return;

  if (!force) {
    var cached = networkReadCache();
    if (cached) {
      networkAllProjects = cached.projects;
      networkNextPageUrl = cached.nextPageUrl;
      networkRenderGrid();
      updateLoadMoreButton();
      setOnlineStatus(networkAllProjects.length + ' project(s) — from a recent check, hit Refresh for the latest.', 'ok');
      return;
    }
  }
  ...
  fetch(networkApiUrl(), { headers: { Accept: 'application/vnd.github+json' } })
    ...
    .then(function (issues) {
      ...
      networkRenderGrid();
      updateLoadMoreButton();
      networkWriteCache(networkAllProjects, networkNextPageUrl);
    })
    .catch(function (err) {
      networkAllProjects = [];
      networkNextPageUrl = null;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (refreshBtn) {
    refreshBtn.addEventListener('click', function () { loadCommunityProjects(true); });
  }
